...
export const PANAMA_OFFSET_MS = 5 * 60 * 60 * 1000;

// Número de día de calendario en Panamá. Dos instantes del mismo día dan el
// mismo número, y cambia a medianoche de Panamá, no a medianoche UTC.
export function panamaDay(ms: number): number {
  return Math.floor((ms - PANAMA_OFFSET_MS) / DIA_MS);
}
...
export function relativeDayLabel(publishedAt: string, now: Date): string {
  ...
  const diff = panamaDay(now.getTime()) - panamaDay(t);
